...
//	public static final byte ADDRESS = (byte) 0x69; // AD0 = 1 (VDD)
	...
	private byte[] temp=new byte[READ_BUFFER_SIZE];;
	...
	private void setupDevice() throws InterruptedException, ConnectionLostException {
		...
		write(DLPF_FS, (byte) (FS_SEL_3 | DLPF_CFG_0)); //   0x10, +/- 2000 deg/sec, 256Hz, 8kHz
		
		read(DLPF_FS, 1, readBuffer);
		System.out.println("DLPF_FS = " + readBuffer[0]);
...
//		write(DLPF_FS, (byte) (FS_SEL_3 | DLPF_CFG_3)); // +/- 2000 deg/sec, 42Hz, 1kHz
		 ...
		 * if the application requires reading every sample of data from the ITG-3200 part, 
		 * it is best to enable the raw data ready interrupt (RAW_RDY_EN).
		 */
	...
	protected boolean read(byte register, int length, byte[] values) throws ConnectionLostException, InterruptedException {
		boolean tenBitAddr = false;
		writeBuffer[0] = register;
		
		return i2c.writeRead(Gyro_ADDRESS, tenBitAddr, writeBuffer, 1, readBuffer, length);
	}
	...
	public void loop() throws ConnectionLostException, InterruptedException {
		if (listener != null) {
			...
			//System.out.println("Gyro read start");
			...
			//System.out.println("Gyro listener onData");
			
			listener.onData(x, y, z, t);
			
			//System.out.println("Gyro read end");
		}
	}
